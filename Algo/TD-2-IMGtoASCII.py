...
def convertImageToAscii(imageFilename, txtFilename, blocSize, mode="txt"):
    ...
    #print(nbBlocVertical, nbBlocHorizontal)
    ...
    for indexBlocVertical in range(nbBlocVertical):
        for indexBlocHorizontal in range(nbBlocHorizontal):
            ...
            if mode =="html":
                ascii += "<span style='background-color: rgb("+str(brightness)+"," + str(brightness)+"," + str(brightness)+");'>"
            ascii += char
            if mode =="html":
                ascii += "</span>"
...
convertImageToAscii("worm.png", "worm.txt",)
