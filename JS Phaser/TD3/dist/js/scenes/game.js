class Game extends Phaser.Scene {
    ...
    create() {
        ...
        for (let couche = 1; couche <= 3; couche++) {
            ...
            for (let t = 0; t < 5; t++) {
                let cloud = this.add.image(
                    ...
                    (Math.random() * window.config.height ) / 3, // Position y aléatoire sur la map
        ...
        for (let couche = 1; couche <= 3; couche++) {
            ...
            for (let t = 0; t < 5; t++) {
                let piece = this.add.image(
                    Math.random() * window.config.width - 300 * 0.2 * couche, // Position x aléatoire sur la map
                    (Math.random() * window.config.height -46 ), // Position y aléatoire sur la map
                    'piece', // Texture du nuage
                )
                    .setOrigin(0, 0) // On défini l'origine des transformations au coin en haut à gauche
                    .setScale((Math.random() * 0.05 + 0.05) * couche);
                // on rédimensionne le nuage aléatoirement par rapport à sa couche, plus la couche est grande
        ...
        this.physics.add.collider(this.player, this.piece);
    ...
    update(time, delta) {
        ...
        this.pieces.forEach((piece) => {
            piece.x -= (delta * 30) / 100 * piece.scale;
            if(piece.x < 0 - piece.width) {
                piece.x = window.config.width;
            }
        });

        
        if (this.player.body.touching.piece){
            window.score ++;
